...
function getCartItems() {
  ...
  fetch('https://falling-maybe-internet-encourage.trycloudflare.com/api/get-cart-details')
  .then(response => response.json())
  .then(formData => {
...
/*function getCartItems() {
  return fetch('https://falling-maybe-internet-encourage.trycloudflare.com/api/get-cart-details')
    .then(response => response.json())
    .then(formData => {
      // Extract the cart items from the response
      return formData.cart.items.map(item => ({
        title: item.title,
        price: item.price
      }));
    })
